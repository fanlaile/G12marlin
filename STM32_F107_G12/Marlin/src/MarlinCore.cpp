...
#include "lcd/ultralcd.h"
  ...
  #include "lcd/dwin/lcd_dwin_ct300.h"
#endif
...
extern bool emergency_stop_status;
...
inline void manage_inactivity(const bool ignore_stepper_queue=false) {
  ...
  if (gcode.stepper_max_timed_out(ms)) {
    ...
    #ifdef RTS_AVAILABLE
    Error_Mode_Handle(Error_Mode_201);
    #else
    kill();
    #endif
  }
...
void idle(TERN_(ADVANCED_PAUSE_FEATURE, bool no_stepper_sleep/*=false*/)) {
  ...
  // Handle UDisk insert / remove
  TERN_(USB_DISK_SUPPORT, Sd2Card::idle());
  ...
  // Handle UI input / draw events
  TERN_(DWIN_CREALITY_RTS, HMI_UpdateProcess());

  // Handle UI input / draw events
  TERN_(RTS_AVAILABLE, LCD_Update());
...
void minkill(const bool steppers_off/*=false*/) {
  ...
  for (int i = 10; i--;) DELAY_US(600);

  cli(); // Stop interrupts

  // Wait to ensure all interrupts stopped
  for (int i = 10; i--;) DELAY_US(250);
  ...
  TERN_(HAS_SUICIDE, suicide());
    ...
    for (;;) 
    {
      ...
      if(READ(E_STOP_PIN) == 1 && emergency_stop_status)   //判断是否拨松紧急停止按钮，若已拨起，系统进行复位
      {
...
void setup() {
  ...
  #if HAS_MULTI_SERIAL || defined(SERIAL_PORT_2)
  ...
  if (mcu & RST_SOFTWARE) 
  {
    ...
    dwinlcd.UART_SndData(0, 0x1000);
    dwinlcd.UART_SndData(0, 0x1001);
    dwinlcd.UART_SndData(ExchangePageBase + 0, ExchangepageAddr);
  }
  ...
  #elif ENABLED(DWIN_CREALITY_RTS)
    SETUP_RUN(HMI_UpdateProcess());
  #else
    SETUP_RUN(ui.init());
  ...
  SETUP_RUN(stepper.init());          // Init stepper. This enables interrupts!
    ...
    #ifdef BASEBOARD_FAN_PIN
      OUT_WRITE(BASEBOARD_FAN_PIN, HIGH);
    #endif
