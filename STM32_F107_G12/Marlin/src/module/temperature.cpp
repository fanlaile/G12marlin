...
void Temperature::min_temp_error(const heater_id_t heater_id) {
  ...
  float Temperature::get_pid_output_hotend(const uint8_t E_NAME) {
    const uint8_t ee = HOTEND_INDEX;
        ...
        //TERN_(RTS_AVAILABLE, rtscheck.heater_heating_or_cooling_done(temp_hotend, ee, pid_error)); Jie
  ...
  float Temperature::get_pid_output_bed() {
      ...
      //TERN_(RTS_AVAILABLE, rtscheck.bed_heating_or_cooling_done(temp_bed, pid_error)); Jie
  ...
  void Temperature::print_heater_states(const uint8_t target_extruder
    ...
    bool Temperature::wait_for_hotend(const uint8_t target_extruder, const bool no_wait_for_cooling/*=true*/
      ...
      if (wait_for_heatup) {
        wait_for_heatup = false;
          ...
          ui.reset_status();
        #endif
        //TERN_(RTS_AVAILABLE, rtscheck.hotend_reset_status());  Jie
    ...
    bool Temperature::wait_for_bed(const bool no_wait_for_cooling/*=true*/
      ...
      if (wait_for_heatup) {
        wait_for_heatup = false;
        ui.reset_status();
        //TERN_(RTS_AVAILABLE, rtscheck.bed_reset_status()); Jie
        return true;
      }
