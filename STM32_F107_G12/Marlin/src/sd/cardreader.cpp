...
uint16_t CardReader::longFilenameUnicode[LONG_FILENAME_LENGTH];
...
int CardReader::countItems(SdFile dir) {
  dir_t p;
  int c = 0;
  while (dir.readDirFile(&p, longFilename,longFilenameUnicode) > 0)
    c += is_dir_or_gcode(p);
...
void CardReader::selectByIndex(SdFile dir, const uint8_t index) {
  dir_t p;
  for (uint8_t cnt = 0; dir.readDirFile(&p, longFilename,longFilenameUnicode) > 0;) {
