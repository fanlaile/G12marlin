 ...
 * a directory file or an I/O error occurred.
 */
int8_t SdBaseFile::readDirFile(dir_t* dir, char* longFilename,uint16_t* longFilenameUnicode) {
  int16_t n;
  // if not a directory file or miss-positioned return an error
  if (!isDir() || (0x1F & curPosition_)) return -1;
    ...
    if (longFilename && DIR_IS_LONG_NAME(dir)) {
      ...
      if (VFAT->firstClusterLow == 0) {
        ...
        if (WITHIN(seq, 1, MAX_VFAT_ENTRIES)) {
          ...
          {
            longFilename[n + i] = (i < 5) ? VFAT->name1[i] : (i < 11) ? VFAT->name2[i - 5] : VFAT->name3[i - 11];
            longFilenameUnicode[n + i] = (i < 5) ? VFAT->name1[i] : (i < 11) ? VFAT->name2[i - 5] : VFAT->name3[i - 11];
          }
          // If this VFAT entry is the last one, add a NUL terminator at the end of the string
          if (VFAT->sequenceNumber & 0x40)
          { 
            longFilename[n + FILENAME_LENGTH] = '\0';
            longFilenameUnicode[n + FILENAME_LENGTH] = '\0';
          }
        }
      }
    }
